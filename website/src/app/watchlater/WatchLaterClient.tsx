...
export default function WatchLaterClient() {
  ...
  const fetchAnimes = useCallback(async () => {
    ...
    try {
      ...
      if (user?.id) {
        res = await fetch("/api/user/watchlater/list", {
          method: "POST",
          headers: {
        ...
        const watchLater = watchLaterRaw ? JSON.parse(watchLaterRaw) : [];

        res = await fetch("/api/guest/watchlater/list", {
          method: "POST",
      ...
      if (Array.isArray(data)) {
        if (data.length === 0) {
          ...
          const parsedData: BaseAnime[] = data.map((item: LikeAnime) => ({
          ...
          setAnimes((prev) => [...prev, ...parsedData]);
      ...
      console.error("Erreur lors du chargement des watch laters :", error);
    } finally {
      setLoading(false);
      ...
      <Grid container spacing={2} justifyContent="center">
        ...
        {!loading && animes.length === 0 && (
          <Grid item xs={12}>
            ...
            >
              Aucun anime enregistré pour plus tard.
            </Typography>
          </Grid>
        )}
      </Grid>
