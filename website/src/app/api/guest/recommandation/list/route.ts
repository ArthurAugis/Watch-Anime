...
export async function POST(req: NextRequest) {
  try {
    ...
    const { likedAnimes, limit = 100, offset = 0 } = body;

    if (!likedAnimes || !Array.isArray(likedAnimes)) {
      return NextResponse.json({ message: "Paramètre likedAnimes manquant ou invalide" }, { status: 400 });
    }

    const decodedLiked = likedAnimes.map((name: string) => decodeURIComponent(name));
    const jsonString = JSON.stringify(decodedLiked);

    const [results]: any = await db.query("CALL get_recommandations(?, ?, ?)", [
      jsonString,
      offset,
      limit,
    ]);
